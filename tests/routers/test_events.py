...
class TestEventsRouter:
    ...
    def test_create_event_duplicate_ssid(self, client, sample_event_data):
        """重複SSIDでのイベント作成失敗テスト"""
        # 最初の作成
        response1 = client.post("/api/v1/events/", json=sample_event_data)
        assert response1.status_code == status.HTTP_201_CREATED

        # 重複SSIDでの作成
        response2 = client.post("/api/v1/events/", json=sample_event_data)
    ...
    def test_get_event_success(self, client, sample_event_data):
        ...
        # イベントを取得
        response = client.get(f"/api/v1/events/{created_event['id']}")
    ...
    def test_update_event_success(self, client, sample_event_data):
        ...
        # イベントを更新
        response = client.put(f"/api/v1/events/{created_event['id']}", json=update_data)
    ...
    def test_update_event_invalid_data(self, client, sample_event_data):
        ...
        response = client.put(f"/api/v1/events/{created_event['id']}", json=invalid_update_data)
    ...
    def test_delete_event_success(self, client, sample_event_data):
        ...
        # イベントを削除
        response = client.delete(f"/api/v1/events/{created_event['id']}")

        assert response.status_code == status.HTTP_204_NO_CONTENT

        # 削除確認
        get_response = client.get(f"/api/v1/events/{created_event['id']}")
    ...
    def test_event_crud_workflow(self, client):
        ...
        update_response = client.put(f"/api/v1/events/{created_event['id']}", json=update_data)
        ...
        # 5. イベントを削除
        delete_response = client.delete(f"/api/v1/events/{created_event['id']}")
        assert delete_response.status_code == status.HTTP_204_NO_CONTENT

        # 6. 削除確認
        final_get_response = client.get(f"/api/v1/events/{created_event['id']}")
