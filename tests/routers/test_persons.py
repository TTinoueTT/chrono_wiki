...
class TestPersonsRouter:
    ...
    def test_create_person_duplicate_ssid(self, client, sample_person_data):
        """重複SSIDでの人物作成失敗テスト"""
        # 最初の作成
        response1 = client.post("/persons/", json=sample_person_data)
        assert response1.status_code == status.HTTP_201_CREATED

        # 重複SSIDでの作成
        response2 = client.post("/persons/", json=sample_person_data)
    ...
    def test_get_persons_with_data(self, client, sample_person_data):
        """データありの人物一覧取得テスト"""
        # 人物を作成
        create_response = client.post("/persons/", json=sample_person_data)
    ...
    def test_get_persons_pagination(self, client):
        ...
        # 最初の2件を取得
        response1 = client.get("/persons/?skip=0&limit=2")
        assert response1.status_code == status.HTTP_200_OK
        data1 = response1.json()
        assert len(data1) <= 2

        # 次の2件を取得
        response2 = client.get("/persons/?skip=2&limit=2")
    ...
    def test_get_person_success(self, client, sample_person_data):
        ...
        # 人物を取得
        response = client.get(f"/persons/{created_person['id']}")
    ...
    def test_get_person_by_ssid_success(self, client, sample_person_data):
        ...
        # SSIDで取得
        response = client.get(f"/persons/ssid/{sample_person_data['ssid']}")
    ...
    def test_get_person_by_ssid_not_found(self, client):
        """存在しないSSIDでの人物取得テスト"""
        response = client.get("/persons/ssid/non_existent_ssid")
    ...
    def test_update_person_success(self, client, sample_person_data):
        ...
        # 人物を更新
        response = client.put(f"/persons/{created_person['id']}", json=update_data)
    ...
    def test_update_person_invalid_data(self, client, sample_person_data):
        ...
        response = client.put(f"/persons/{created_person['id']}", json=invalid_update_data)
    ...
    def test_delete_person_success(self, client, sample_person_data):
        ...
        # 人物を削除
        response = client.delete(f"/persons/{created_person['id']}")

        assert response.status_code == status.HTTP_204_NO_CONTENT

        # 削除確認
        get_response = client.get(f"/persons/{created_person['id']}")
    ...
    def test_person_crud_workflow(self, client):
        ...
        # 3. SSIDで人物を取得
        ssid_response = client.get(f"/persons/ssid/{person_data['ssid']}")
        ...
        update_response = client.put(f"/persons/{created_person['id']}", json=update_data)
        ...
        # 6. 人物を削除
        delete_response = client.delete(f"/persons/{created_person['id']}")
        assert delete_response.status_code == status.HTTP_204_NO_CONTENT

        # 7. 削除確認
        final_get_response = client.get(f"/persons/{created_person['id']}")
