...
class TestAuthErrorScenarios:
    ...
    def test_login_account_locked(self, auth_client):
        ...
        response = auth_client.post("/api/v1/auth/login", data=correct_login_data)
        assert response.status_code == status.HTTP_423_LOCKED

    def test_refresh_token_invalid_token(self, auth_client):
        """無効なリフレッシュトークンのテスト"""
        response = auth_client.post("/api/v1/auth/refresh", json={"refresh_token": "invalid_token"})
        assert response.status_code == status.HTTP_401_UNAUTHORIZED
    ...
    def test_refresh_token_wrong_type(self, auth_client):
        ...
        response = auth_client.post("/api/v1/auth/refresh", json={"refresh_token": access_token})
        assert response.status_code == status.HTTP_401_UNAUTHORIZED
    ...
    def test_refresh_token_inactive_user(self, auth_client):
        ...
        auth_client.post("/api/v1/auth/register", json=admin_data)

        admin_login_data = {
            "username": admin_data["email"],
            "password": admin_data["password"],
        }
        admin_login_response = auth_client.post("/api/v1/auth/login", data=admin_login_data)
        admin_token = admin_login_response.json()["access_token"]
        admin_headers = {"Authorization": f"Bearer {admin_token}"}

        # ユーザーを無効化
        auth_client.post(f"/api/v1/users/{user_id}/deactivate", headers=admin_headers)

        # 無効化されたユーザーのリフレッシュトークンを使用
        response = auth_client.post("/api/v1/auth/refresh", json={"refresh_token": refresh_token})
        assert response.status_code == status.HTTP_401_UNAUTHORIZED

    def test_refresh_token_with_missing_sub(self, auth_client):
        """subフィールドが欠けているリフレッシュトークンのテスト"""
        response = auth_client.post("/api/v1/auth/refresh", json={"refresh_token": "invalid_token_missing_sub"})
        assert response.status_code == status.HTTP_401_UNAUTHORIZED

    def test_refresh_token_with_none_sub(self, auth_client):
        """subフィールドがNoneのリフレッシュトークンのテスト"""
        response = auth_client.post("/api/v1/auth/refresh", json={"refresh_token": "invalid_token_none_sub"})
        assert response.status_code == status.HTTP_401_UNAUTHORIZED
...
class TestAuthEdgeCases:
    ...
    def test_successful_login_resets_failed_attempts(self, auth_client):
        ...
        for _ in range(2):
            ...
            auth_client.post("/api/v1/auth/login", data=login_data)
        ...
        response = auth_client.post("/api/v1/auth/login", data=wrong_login_data)
        assert response.status_code == status.HTTP_401_UNAUTHORIZED
    ...
    def test_register_with_invalid_data(self, auth_client):
        ...
        response = auth_client.post("/api/v1/auth/register", json=invalid_user_data)
    ...
    def test_update_user_with_invalid_data(self, auth_client):
        ...
        response = auth_client.put("/api/v1/auth/me", json=invalid_update_data, headers=headers)
        assert response.status_code == status.HTTP_422_UNPROCESSABLE_ENTITY
...
class TestAuthSuccessScenarios:
    ...
    def test_successful_user_update(self, auth_client):
        ...
        response = auth_client.put("/api/v1/auth/me", json=update_data, headers=headers)
        assert response.status_code == status.HTTP_200_OK
        data = response.json()
    ...
    def test_successful_logout(self, auth_client):
        ...
        # ログアウト
        response = auth_client.post("/api/v1/auth/logout", headers=headers)
        assert response.status_code == status.HTTP_200_OK
...
class TestAuthParameterizedScenarios:
    ...
    def test_user_registration_scenarios(self, auth_client, scenario):
        ...
        if scenario["name"] in ["duplicate_email", "duplicate_username"]:
            ...
            first_response = auth_client.post("/api/v1/auth/register", json=first_user_data)
            assert first_response.status_code == status.HTTP_201_CREATED

        response = auth_client.post("/api/v1/auth/register", json=scenario["data"])
    ...
    def test_user_login_scenarios(self, auth_client, scenario):
        """ログインの様々なシナリオをテスト"""
        response = auth_client.post("/api/v1/auth/login", data=scenario["login_data"])
